...
export function App() {
  ...
  return <main>
    ...
    <section className="news-section" id="news"><div className="container"><div className="section-heading-row"><div><p className="eyebrow teal">NEWS &amp; EVENTS</p><h2 className="section-title">Follow the project as it takes shape.</h2></div><button className="text-button" onClick={() => navTo("opportunities")}>Ask about opportunities <FaArrowRight /></button></div><div className="news-grid">{[["September 2026", "NORDIC-EMS project launch", "The consortium begins aligning research activities, mobility plans and first-year priorities."], ["October 2026", "NORDIC-EMS kick-off meeting", "Partners align research, mobility, governance and first-year priorities."], ["Q4 2026", "First mobility activities", "The consortium begins planning the initial PhD and researcher exchanges."], ["2027", "Nordic Summer School planning", "The programme develops a shared learning experience across the Nordic research ecosystem."]].map(([date, title, text]) => <article key={title}><p>{date}</p><h3>{title}</h3><span /><div>{text}</div><button onClick={() => navTo("opportunities")}>Read more <FaArrowRight /></button></article>)}</div></div></section>
