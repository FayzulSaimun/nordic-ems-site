...
const projectTargets = [
  [FaUsers, "12", "Mobility instances", "PhD and researcher collaboration"],
...
const workPackages = [
  ...
  ["WP5", "Communication, Knowledge Exchange & Societal Impact", "Lead: LTU · M1–M40", "Connects project research and mobility activities with industry, communities, public stakeholders and society. WP5 supports co-creation, dissemination, stakeholder engagement, policy and industry dialogue, public outreach, and the uptake of project results beyond academia."],
];
...
const universities = [
  ["HIOF", "Østfold University of Applied Sciences", "Norway", "hiof", "hiof-2026.jpeg", "Intelligent control, energy storage, digitalization & AI-enabled energy systems"],
...
export function App() {
  ...
  return <main>
    ...
    <section className="numbers reference-numbers" id="about"><div className="container"><h2 className="numbers-title">NORDIC-EMS IN NUMBERS</h2><h3 className="metrics-group-title">Consortium composition</h3><div className="number-grid consortium-facts">{consortiumFacts.map(([Icon, number, label, detail]) => <article className="number-item" key={label}><Icon className="number-icon" /><strong>{number}</strong><h3>{label}</h3><p>{detail}</p></article>)}</div><h3 className="metrics-group-title targets-title">Project targets for 2026–2029</h3><div className="number-grid project-targets">{projectTargets.map(([Icon, number, label, detail]) => <article className="number-item" key={label}><Icon className="number-icon" /><strong>{number}</strong><h3>{label}</h3><p>{detail}</p></article>)}</div><p className="kpi-line"><strong>Additional project targets:</strong> 3–4 thematic research clusters · 2 pilot-oriented collaborations · 2 co-supervised PhDs · 1 Nordic Summer School · 2 policy / industry briefs</p></div></section>
